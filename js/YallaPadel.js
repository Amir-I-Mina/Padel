...
function loadProducts() {
    const savedProducts = localStorage.getItem('padelProducts');
    if (savedProducts) {
        products = JSON.parse(savedProducts);
    } else {
        // Default products
        products = [
            { id: 'racket', name: 'Pro Carbon Padel Racket', price: 1000, desc: 'Professional-grade carbon fiber racket', hasOptions: false, inStock: true, image: 'racketphoto.png' },
            { id: 'balls', name: 'Premium Ball Pack (3 Cans)', price: 350, desc: 'Official padel balls, pressure-sealed', hasOptions: false, inStock: true, image: 'ballsphoto.png' },
            { id: 'kit', name: 'Elite Padel Sport Kit', price: 1500, desc: 'Complete sport kit, breathable fabric', hasOptions: true, inStock: true, image: 'kitphoto.png', sizes: ['S','M','L','XL'], colors: ['White','Black','Navy'] }
        ];
        saveProducts();
    }
    displayProducts();
...
function saveProducts() {
    localStorage.setItem('padelProducts', JSON.stringify(products));
}
        ...
        if (p.hasOptions && p.inStock !== false) {
            optionsHtml = `
                <div class="options-group">
                    ...
                    <select id="size_${p.id}" class="size-select">
                        ${p.sizes.map(s => `<option value="${s}">${s}</option>`).join('')}
                    </select>
                </div>
                <div class="options-group">
                    <label class="options-label">Color:</label>
                    <select id="color_${p.id}" class="color-select">
        ...
        grid.innerHTML += `
            <div class="product-card ${outOfStockClass}" data-id="${p.id}">
                <div class="card-image">
                    <img src="${p.image}" style="width:100%;height:100%;object-fit:cover;" onerror="this.src='placeholder.png'">
                </div>
                <div class="card-content">
                    ...
                    <div class="quantity-selector">
                        <button class="qty-btn" onclick="changeQty('${p.id}', -1)" ${disabledAttr}>-</button>
                        <input type="number" id="qty_${p.id}" class="qty-input" value="1" min="1" readonly>
                        <button class="qty-btn" onclick="changeQty('${p.id}', 1)" ${disabledAttr}>+</button>
                    </div>
                    <button class="btn-action" onclick="addToCart('${p.id}')" ${disabledAttr}>ADD TO CART</button>
                </div>
            </div>
        `;
    }
...
function addProduct() {
    ...
    const hasOptions = document.getElementById('productHasOptions')?.value === 'true';
    ...
    if (hasOptions) {
        newProduct.sizes = ['S','M','L','XL'];
        newProduct.colors = ['White','Black','Navy'];
    }
